...
World Bank definition: GDP per capita > $13,845 (current USD)

This version uses:
1. Pooled periods instead of year FEs (to handle small sample)
2. Proper sample adequacy checks
3. Bootstrap with error handling
4. Comparison between full sample and high-income subsample
...
LOWER_MIDDLE_THRESH = 1135

# Choose your analysis:
GDP_THRESH = HIGH_INCOME_THRESH  # High-income only
# GDP_THRESH = UPPER_MIDDLE_THRESH  # Upper-middle + High-income
# GDP_THRESH = LOWER_MIDDLE_THRESH  # All except low-income

N_BOOTSTRAP = 1000  # Will use less for high-income sample due to small n

# Minimum observations needed (n >= 10 * k)
...
def validate_fpe_scores(fpe_data):
    ...
    if n_invalid > 0:
        ...
        # Check if all are exactly 1.0 (floating point issue)
        if (invalid_data['fpe_score'] == 1.0).all():
            print("\n  ✅ All 'invalid' scores are exactly 1.0")
            print("  ✅ Treating as valid frontier observations")
            # Actually, they ARE valid - fix the mask
...
def build_stage2_panel():
    """Build full and restricted panels"""
    print("Loading data...")
    ...
    # Also create a less restrictive sample for comparison
    panel_upper_middle = panel_full[panel_full['gdp_pc'] >= UPPER_MIDDLE_THRESH].copy()
    panel_high_income = panel_full[panel_full['gdp_pc'] >= HIGH_INCOME_THRESH].copy()

    print(f"\nSample Sizes:")
    print(f"  Full sample          : {len(panel_full):>4} obs, {panel_full['iso3'].nunique():>2} countries")
    print(
        f"  Upper-middle + high  : {len(panel_upper_middle):>4} obs, {panel_upper_middle['iso3'].nunique():>2} countries")
    print(
        f"  High-income only     : {len(panel_high_income):>4} obs, {panel_high_income['iso3'].nunique():>2} countries")

    return panel_full, panel_upper_middle, panel_high_income


# =============================================================================
# DESIGN MATRIX (SIMPLIFIED FOR HIGH-INCOME)
# =============================================================================

def build_design_matrix_simple(df):
    """
    Simplified design matrix for high-income countries:
    - No year fixed effects (too few observations)
    - Uses time trend instead
    """
    d = df.copy()
    d['wgi_std'] = (d['wgi_composite'] - d['wgi_composite'].mean()) / d['wgi_composite'].std()
    d['gini_std'] = (d['gini'] - d['gini'].mean()) / d['gini'].std()

    # Use time trend instead of year FEs
    X = pd.DataFrame({
        'const': 1.0,
...
def build_design_matrix_pooled(df):
    """
    Pool years into 5-year periods
    """
    d = df.copy()
    d['wgi_std'] = (d['wgi_composite'] - d['wgi_composite'].mean()) / d['wgi_composite'].std()
    d['gini_std'] = (d['gini'] - d['gini'].mean()) / d['gini'].std()
    ...
    # Add period dummies (drop first as baseline)
...
def build_design_matrix_adaptive(df):
    """
    Adaptive design matrix based on sample size
    """
    n_obs = len(df)
    n_years = df['year'].nunique()

    # If we have enough observations, use year FEs
    if n_obs >= 10 * (3 + n_years - 1):
        print(f"    Using year fixed effects (n={n_obs}, k={3 + n_years - 1})")
        return build_design_matrix_pooled(df)  # Placeholder - implement full year FE version
    else:
        # Otherwise use pooled periods
        print(f"    Using pooled periods (n={n_obs}, fewer parameters)")
        return build_design_matrix_pooled(df)
...
# =============================================================================
# BOOTSTRAP (ADAPTIVE FOR SMALL SAMPLES)
# =============================================================================
...
def run_stage2_high_income(panel, label, design_method='pooled'):
    """
    Run Stage 2 specifically for high-income analysis
    """
    print(f"\n{'=' * 65}")
...
def compare_results(results_list):
    ...
    print(f"\n  {'Sample':<20} {'n':>6} {'n/k':>6} {'WGI β':>10} {'WGI p':>8} "
    ...
    for res in valid_results:
        ...
        print(f"  {res['label']:<20} {res['n_trunc']:>6} {res['n_per_param']:>6.1f} "
    ...
    pd.DataFrame(rows).to_csv('stage2_high_income_results.csv', index=False)
    print(f"\n  Results saved to 'stage2_high_income_results.csv'")
...
def descriptive_stats(panel_full, panel_high_income):
    """Generate descriptive statistics for the samples"""
    print(f"\n{'=' * 65}")
    ...
    for name, panel in [('Full Sample', panel_full),
                        ('High-Income Only', panel_high_income)]:
        if len(panel) == 0:
            continue
    ...
    # Save
    df_stats.to_csv('stage2_descriptive_stats.csv', index=False)
    print("\n  Descriptive statistics saved to 'stage2_descriptive_stats.csv'")
...
def main():
    """Run the complete high-income analysis pipeline"""
    print("\n" + "=" * 65)
    print("  SIMAR-WILSON STAGE 2: HIGH-INCOME COUNTRIES ANALYSIS")
    print("=" * 65)
    ...
    # Descriptive statistics
    descriptive_stats(panel_full, panel_high_income)

    # Run Stage 2 on different samples
    results = []

    # Full sample (using standard approach)
    print(f"\n{'-' * 65}")
    print("  ANALYSIS 1: FULL SAMPLE")
    print(f"{'-' * 65}")
    res_full = run_stage2_high_income(panel_full, 'Full Sample', design_method='pooled')
    results.append(res_full)

    # Upper-middle + High income
    if len(panel_upper_middle) > 50:
        print(f"\n{'-' * 65}")
        print("  ANALYSIS 2: UPPER-MIDDLE + HIGH INCOME")
        print(f"{'-' * 65}")
        res_upper = run_stage2_high_income(panel_upper_middle, 'Upper-Middle + High Income', design_method='pooled')
        results.append(res_upper)

    # High-income only
    if len(panel_high_income) > 30:
        print(f"\n{'-' * 65}")
        print("  ANALYSIS 3: HIGH-INCOME ONLY")
        print(f"{'-' * 65}")
        res_high = run_stage2_high_income(panel_high_income, 'High-Income Only', design_method='pooled')
    ...
    print("  SUMMARY")
    print(f"{'=' * 65}")
    print(f"  Threshold used: ${GDP_THRESH:,}")
    print(f"  High-income countries: {panel_high_income['iso3'].nunique()}")
    print(f"  High-income observations: {len(panel_high_income)}")
    print("  See 'stage2_high_income_results.csv' for detailed results")
    print("=" * 65)
